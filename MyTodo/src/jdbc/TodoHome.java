...
// Generated 12 févr. 2014 12:41:52 by Hibernate Tools 4.0.0
...
/**
 * Home object for domain model class Todo.
 * @see jdbc.Todo
 * @author Hibernate Tools
 */
public class TodoHome {

	private static final Log log = LogFactory.getLog(TodoHome.class);
	...
	public void persist(Todo transientInstance) {
		log.debug("persisting Todo instance");
		try {
	...
	public Todo merge(Todo detachedInstance) {
		...
		try {
			Todo result = (Todo) sessionFactory.getCurrentSession().merge(
					detachedInstance);
	...
	public Todo findById(java.lang.Integer id) {
		log.debug("getting Todo instance with id: " + id);
		try {
			Todo instance = (Todo) sessionFactory.getCurrentSession().get(
					"jdbc.Todo", id);
			if (instance == null) {
	...
	public List findByExample(Todo instance) {
		...
		try {
			List results = sessionFactory.getCurrentSession()
					.createCriteria("jdbc.Todo").add(Example.create(instance))
					.list();
